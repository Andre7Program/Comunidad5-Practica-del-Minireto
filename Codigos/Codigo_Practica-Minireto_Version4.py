...
class ImageProcessingApp:
    ...
    def setup_ui(self):
        ...
        ttk.Button(load_frame, text="Eliminar Imagen 1", command=self.delete_image1).grid(row=1, column=0, padx=2, pady=2)
        ttk.Button(load_frame, text="Eliminar Imagen 2", command=self.delete_image2).grid(row=1, column=1, padx=2, pady=2)
        ttk.Button(load_frame, text="Eliminar Todas", command=self.delete_all_images).grid(row=1, column=2, padx=2, pady=2)
        ...
        arith_scalar_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=4)
        ...
        arith_images_frame.grid(row=2, column=0, sticky=(tk.W, tk.E), pady=4)
        ...
        # Operaciones lógicas
        logic_frame = ttk.LabelFrame(controls_frame, text="Operaciones Lógicas", padding=4)
        ...
        # Preprocesamiento
        preprocess_frame = ttk.LabelFrame(controls_frame, text="Preprocesamiento", padding=4)
        ...
        labeling_frame = ttk.LabelFrame(controls_frame, text="Etiquetado de Componentes Conexas", padding=4)
        ...
        # Filtros lineales (Pasa-bajas) y Filtros no lineales (de orden)
        filters_frame = ttk.LabelFrame(controls_frame, text="Filtros Lineales y No Lineales", padding=4)
        filters_frame.grid(row=6, column=0, sticky=(tk.W, tk.E), pady=6)
        ttk.Button(filters_frame, text="Filtro Promediador", command=self.filter_mean).grid(row=0, column=0, padx=2, pady=3)
        ttk.Button(filters_frame, text="Filtro Promediador Pesado", command=self.filter_weighted_mean).grid(row=0, column=1, padx=2, pady=3)
        ttk.Button(filters_frame, text="Filtro Gaussiano", command=self.filter_gaussian).grid(row=0, column=2, padx=2, pady=3)
        ttk.Button(filters_frame, text="Filtro Mediana", command=self.filter_median).grid(row=0, column=3, padx=2, pady=3)
        ttk.Button(filters_frame, text="Filtro Moda", command=self.filter_mode).grid(row=1, column=0, padx=2, pady=3)
        ttk.Button(filters_frame, text="Filtro Bilateral", command=self.filter_bilateral).grid(row=1, column=1, padx=2, pady=3)
        ttk.Button(filters_frame, text="Filtro Maximo", command=self.filter_maximum).grid(row=1, column=2, padx=2, pady=3)
        ttk.Button(filters_frame, text="Filtro Minimo", command=self.filter_minimum).grid(row=1, column=3, padx=2, pady=3)
        
        #Ruidos
        ruidos_frame = ttk.LabelFrame(controls_frame, text="Ruidos", padding=4)
        ruidos_frame.grid(row=7, column=0, sticky=(tk.W, tk.E), pady=6)
        ttk.Button(ruidos_frame, text="Ruido Sal y Pimienta", command=self.add_salt_pepper_noise).grid(row=0, column=0, padx=2, pady=3)
        ttk.Button(ruidos_frame, text="Ruido Gaussiano", command=self.add_gaussian_noise).grid(row=0, column=1, padx=2, pady=3)

        # visualización controles (derecha)
    ...
    def arithmetic_operation_scalar(self, operation):
        ...
        if operation == 'add':
            value = self.get_numeric_input("Suma con Escalar", "Ingresa el valor a sumar a cada píxel:", 50)
            if value is None: return
            # cv2.add maneja saturación
            self.processed_image = cv2.add(src, np.array(value, dtype=src.dtype))
            self.set_message(f"Suma con escalar +{value} aplicada.")
        elif operation == 'subtract':
            value = self.get_numeric_input("Resta con Escalar", "Ingresa el valor a restar a cada píxel:", 50)
            if value is None: return
            self.processed_image = cv2.subtract(src, np.array(value, dtype=src.dtype))
    ...
    def arithmetic_operation_images(self, operation):
        ...
        # A y B pueden ser color o gris; usar operaciones seguras
        try:
            if operation == 'add':
                result = cv2.add(A, B); self.set_message("Suma de imágenes aplicada.")
            elif operation == 'subtract':
                result = cv2.subtract(A, B); self.set_message("Resta de imágenes aplicada.")
            elif operation == 'multiply':
                result = cv2.multiply(A, B); self.set_message("Multiplicación de imágenes aplicada.")
            else:
    ...
    def logical_operation(self, operation):
        ...
        if operation != 'not':
            ...
            # NOT sobre imagen original (si color, invertir cada canal)
    ...
    def custom_threshold(self):
        ...
        threshold_value = int(max(0, min(255, threshold_value)))
    ...
    # ----------------- Filtros y Ruido (aplican sobre la imagen original) -----------------
    ...
    def filter_maximum(self):
        ...
        self.processed_image = img_max
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Filtro de Máximo aplicado.")
    ...
    def filter_minimum(self):
        ...
        # Mostrar los resultados
        self.processed_image = img_min
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Filtro de Minimo aplicado.")
    ...
    def connected_components(self, connectivity):
        ...
        self.set_message(f"Vecindad {connectivity}: {max(0, num_labels-1)} objetos detectados.")
    ...
    def contour_objects(self):
        ...
        for i, contour in enumerate(contours):
            ...
            cv2.putText(result_image, f'{i+1}', (x, max(10, y-5)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
    ...
    def save_processed_image(self):
        ...
        if not file_path:
            return
        # si la imagen es BGR o RGB, cv2.imwrite espera BGR; aquí asumimos que processed_image es BGR o Gray.
