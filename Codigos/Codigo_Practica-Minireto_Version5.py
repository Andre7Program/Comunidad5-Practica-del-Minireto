...
class ImageProcessingApp:
    ...
    def setup_ui(self):
        ...
        ttk.Button(load_frame, text="Guardar Procesada", command=self.save_processed_image).grid(row=0, column=2,
                                                                                                 padx=2)
        ...
        ttk.Button(history_frame, text="Deshacer Último", command=self.undo_last_processing).grid(row=0, column=0,
                                                                                                  padx=2, pady=3)
        ttk.Button(history_frame, text="Ver Historial", command=self.show_processing_history).grid(row=0, column=1,
                                                                                                   padx=2, pady=3)
        ...
        ttk.Button(noise_frame, text="Ruido Sal y Pimienta", command=self.add_salt_pepper_noise).grid(row=0, column=0,
                                                                                                      padx=2, pady=3)
        ttk.Button(noise_frame, text="Ruido Gaussiano", command=self.add_gaussian_noise).grid(row=0, column=1, padx=2,
                                                                                              pady=3)
        ttk.Button(noise_frame, text="Quitar Ruido", command=self.remove_noise_filters).grid(row=0, column=2, padx=2,
                                                                                             pady=3)
        ...
        ttk.Button(lowpass_frame, text="Filtro Promediador", command=self.apply_average_filter).grid(row=0, column=0,
                                                                                                     padx=2, pady=3)
        ttk.Button(lowpass_frame, text="Promediador Pesado", command=self.apply_weighted_average_filter).grid(row=0,
                                                                                                              column=1,
                                                                                                              padx=2,
                                                                                                              pady=3)
        ttk.Button(lowpass_frame, text="Filtro Gaussiano", command=self.apply_gaussian_filter).grid(row=0, column=2,
                                                                                                    padx=2, pady=3)
        ttk.Button(lowpass_frame, text="Filtro Bilateral", command=self.apply_bilateral_filter).grid(row=1, column=0,
                                                                                                     padx=2, pady=3)
        ttk.Button(lowpass_frame, text="Quitar Suavizado", command=self.remove_smoothing_filters).grid(row=1, column=1,
                                                                                                       padx=2, pady=3)
        ...
        ttk.Button(nonlinear_frame, text="Filtro Mediana", command=self.apply_median_filter).grid(row=0, column=0,
                                                                                                  padx=2, pady=3)
        ttk.Button(nonlinear_frame, text="Filtro Moda", command=self.apply_mode_filter).grid(row=0, column=1, padx=2,
                                                                                             pady=3)
        ttk.Button(nonlinear_frame, text="Filtro Máximo", command=self.apply_max_filter).grid(row=0, column=2, padx=2,
                                                                                              pady=3)
        ttk.Button(nonlinear_frame, text="Filtro Mínimo", command=self.apply_min_filter).grid(row=1, column=0, padx=2,
                                                                                              pady=3)
        ttk.Button(nonlinear_frame, text="Quitar No Lineales", command=self.remove_nonlinear_filters).grid(row=1,
                                                                                                           column=1,
                                                                                                           padx=2,
                                                                                                           pady=3)
        ...
        ttk.Button(highpass_frame, text="Laplaciano", command=self.apply_laplacian).grid(row=1, column=2, padx=2,
                                                                                         pady=3)
        ttk.Button(highpass_frame, text="Quitar Bordes", command=self.remove_edge_filters).grid(row=2, column=0, padx=2,
                                                                                                pady=3)
        ...
        ttk.Button(directional_frame, text="Líneas Horizontales",
                   command=lambda: self.apply_directional_filter('horizontal')).grid(row=0, column=0, padx=2, pady=3)
        ttk.Button(directional_frame, text="Líneas Verticales",
                   command=lambda: self.apply_directional_filter('vertical')).grid(row=0, column=1, padx=2, pady=3)
        ttk.Button(directional_frame, text="Diagonal Principal",
                   command=lambda: self.apply_directional_filter('diagonal_main')).grid(row=0, column=2, padx=2, pady=3)
        ttk.Button(directional_frame, text="Diagonal Secundaria",
                   command=lambda: self.apply_directional_filter('diagonal_secondary')).grid(row=1, column=0, padx=2,
                                                                                             pady=3)
        ttk.Button(directional_frame, text="Quitar Direccionales", command=self.remove_directional_filters).grid(row=1,
                                                                                                                 column=1,
                                                                                                                 padx=2,
                                                                                                                 pady=3)
        ...
        ttk.Button(arith_scalar_frame, text="Suma", command=lambda: self.arithmetic_operation_scalar('add')).grid(row=0,
                                                                                                                  column=0,
                                                                                                                  padx=2,
                                                                                                                  pady=3)
        ttk.Button(arith_scalar_frame, text="Resta", command=lambda: self.arithmetic_operation_scalar('subtract')).grid(
            row=0, column=1, padx=2, pady=3)
        ttk.Button(arith_scalar_frame, text="Multiplicación",
                   command=lambda: self.arithmetic_operation_scalar('multiply')).grid(row=0, column=2, padx=2, pady=3)
        ttk.Button(arith_scalar_frame, text="Quitar Aritméticas", command=self.remove_arithmetic_operations).grid(row=0,
                                                                                                                  column=3,
                                                                                                                  padx=2,
                                                                                                                  pady=3)
        ...
        ttk.Button(arith_images_frame, text="Suma", command=lambda: self.arithmetic_operation_images('add')).grid(row=0,
                                                                                                                  column=0,
                                                                                                                  padx=2,
                                                                                                                  pady=3)
        ttk.Button(arith_images_frame, text="Resta", command=lambda: self.arithmetic_operation_images('subtract')).grid(
            row=0, column=1, padx=2, pady=3)
        ttk.Button(arith_images_frame, text="Multiplicación",
                   command=lambda: self.arithmetic_operation_images('multiply')).grid(row=0, column=2, padx=2, pady=3)
        ttk.Button(arith_images_frame, text="Quitar Operaciones", command=self.remove_image_operations).grid(row=0,
                                                                                                             column=3,
                                                                                                             padx=2,
                                                                                                             pady=3)
        ...
        ttk.Button(logic_frame, text="NOT", command=lambda: self.logical_operation('not')).grid(row=0, column=3, padx=2,
                                                                                                pady=3)
        ttk.Button(logic_frame, text="Quitar Lógicas", command=self.remove_logical_operations).grid(row=1, column=0,
                                                                                                    padx=2, pady=3)
        ...
        ttk.Button(preprocess_frame, text="Escala de Grises", command=self.convert_to_grayscale).grid(row=0, column=0,
                                                                                                      padx=2, pady=3)
        ttk.Button(preprocess_frame, text="Umbralizar", command=self.threshold_image).grid(row=0, column=1, padx=2,
                                                                                           pady=3)
        ttk.Button(preprocess_frame, text="Umbral Personalizado", command=self.custom_threshold).grid(row=0, column=2,
                                                                                                      padx=2, pady=3)
        ttk.Button(preprocess_frame, text="Quitar Preprocesamiento", command=self.remove_preprocessing).grid(row=0,
                                                                                                             column=3,
                                                                                                             padx=2,
                                                                                                             pady=3)
        ...
        ttk.Button(labeling_frame, text="Contornear Objetos", command=self.contour_objects).grid(row=0, column=2,
                                                                                                 padx=2, pady=3)
        ttk.Button(labeling_frame, text="Quitar Etiquetado", command=self.remove_labeling).grid(row=0, column=3, padx=2,
                                                                                                pady=3)
    ...
    def remove_smoothing_filters(self):
        ...
        if self.processing_history:
            # Buscar el último estado antes de aplicar suavizado
            # Primero identificar qué operaciones son de suavizado
            smoothing_operations = ['promediador', 'gaussiano', 'bilateral', 'suavizado', 'mediana']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                # Si este estado no es de suavizado, usarlo
                if not any(op in state['name'].lower() for op in smoothing_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
                ...
                # Volver al original si no se encuentra
                self.processed_image = self.original_image.copy() if self.original_image is not None else None
                self.gray_image = None
                self.binary_image = None
                self.processing_history = [{
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Filtros de suavizado removidos")

    def remove_nonlinear_filters(self):
        """Quita solo filtros no lineales"""
        if self.processing_history:
            nonlinear_operations = ['mediana', 'moda', 'máximo', 'mínimo', 'no lineal']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in nonlinear_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Filtros no lineales removidos")

    def remove_edge_filters(self):
        """Quita solo filtros de detección de bordes"""
        if self.processing_history:
            edge_operations = ['sobel', 'prewitt', 'roberts', 'canny', 'kirsch', 'laplaciano', 'bordes']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in edge_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Filtros de bordes removidos")

    def remove_directional_filters(self):
        """Quita solo máscaras direccionales"""
        if self.processing_history:
            directional_operations = ['horizontal', 'vertical', 'diagonal', 'direccional', 'máscara']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in directional_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Máscaras direccionales removidas")

    def remove_arithmetic_operations(self):
        """Quita solo operaciones aritméticas con escalar"""
        if self.processing_history:
            arithmetic_operations = ['suma', 'resta', 'multiplicación', 'aritmética']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in arithmetic_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Operaciones aritméticas removidas")

    def remove_image_operations(self):
        """Quita solo operaciones entre imágenes"""
        if self.processing_history:
            image_operations = ['imágenes', 'operaciones entre']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in image_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Operaciones entre imágenes removidas")

    def remove_logical_operations(self):
        """Quita solo operaciones lógicas"""
        if self.processing_history:
            logical_operations = ['and', 'or', 'xor', 'not', 'lógica']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in logical_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Operaciones lógicas removidas")

    def remove_preprocessing(self):
        """Quita solo preprocesamiento básico (vuelve a color)"""
        if self.processing_history:
            preprocessing_operations = ['grises', 'umbral']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in preprocessing_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Preprocesamiento removido")

    def remove_labeling(self):
        """Quita solo etiquetado de componentes"""
        if self.processing_history:
            labeling_operations = ['vecindad', 'contornear', 'etiquetado', 'componentes']

            for i in range(len(self.processing_history) - 1, -1, -1):
                state = self.processing_history[i]
                if not any(op in state['name'].lower() for op in labeling_operations):
                    self.processed_image = state['image'].copy()
                    self.gray_image = state['gray'].copy() if state['gray'] is not None else None
                    self.binary_image = state['binary'].copy() if state['binary'] is not None else None
        ...
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Etiquetado de componentes removido")
    ...
    def add_salt_pepper_noise(self):
        ...
        self.save_processing_state("Ruido Sal y Pimienta")

        # MANTENER FORMATO ESCALA DE GRISES
        if len(self.original_image.shape) == 2:
            if self.processed_image is not None and len(self.processed_image.shape) == 3:
        ...
        self.processed_image = agregar_ruido_sal_pimienta(image, cantidad)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Ruido Sal y Pimienta aplicado (cantidad={cantidad})")
    ...
    def add_gaussian_noise(self):
        ...
        self.save_processing_state("Ruido Gaussiano")
        ...
        self.processed_image = agregar_ruido_gaussiano(image, 0, sigma)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Ruido Gaussiano aplicado (sigma={sigma})")
    ...
    def apply_average_filter(self):
        ...
        self.save_processing_state("Filtro Promediador")
        ...
        self.processed_image = cv2.blur(input_image, (kernel_size, kernel_size))
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Promediador aplicado (kernel {kernel_size}x{kernel_size})")
    ...
    def apply_weighted_average_filter(self):
        ...
        self.save_processing_state("Filtro Promediador Pesado")
        ...
        self.processed_image = cv2.filter2D(input_image, -1, kernel)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
    ...
    def apply_gaussian_filter(self):
        ...
        self.save_processing_state("Filtro Gaussiano")
        ...
        self.processed_image = cv2.GaussianBlur(input_image, (kernel_size, kernel_size), sigma)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Gaussiano aplicado (kernel {kernel_size}x{kernel_size}, sigma={sigma})")
    ...
    def apply_bilateral_filter(self):
        ...
        self.save_processing_state("Filtro Bilateral")
        ...
        self.processed_image = cv2.bilateralFilter(input_image, d, sigma_color, sigma_space)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Bilateral aplicado (d={d}, sigmaColor={sigma_color}, sigmaSpace={sigma_space})")
    ...
    def apply_median_filter(self):
        ...
        self.save_processing_state("Filtro Mediana")
        ...
        self.processed_image = cv2.medianBlur(input_image, kernel_size)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Mediana aplicado (kernel {kernel_size}x{kernel_size})")
    ...
    def apply_mode_filter(self):
        ...
        self.save_processing_state("Filtro Moda")
        ...
        self.processed_image = filtro_moda_counter(image, kernel_size)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Moda aplicado (kernel {kernel_size}x{kernel_size})")
    ...
    def apply_max_filter(self):
        ...
        self.save_processing_state("Filtro Máximo")
        ...
        self.processed_image = cv2.dilate(input_image, kernel)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Máximo aplicado (kernel {kernel_size}x{kernel_size})")
    ...
    def apply_min_filter(self):
        ...
        self.save_processing_state("Filtro Mínimo")
        ...
        kernel = np.ones((kernel_size, kernel_size), np.uint8)
        self.processed_image = cv2.erode(input_image, kernel)
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Mínimo aplicado (kernel {kernel_size}x{kernel_size})")
    ...
    def apply_directional_filter(self, direction):
        ...
        self.save_processing_state(f"Máscara {direction}")
        ...
        self.processed_image = np.uint8(np.clip(filtered, 0, 255))
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Máscara {title} aplicada")
    ...
    def convert_to_grayscale(self):
        ...
        self.save_processing_state("Escala de Grises")
    ...
    def threshold_image(self):
        self.save_processing_state("Umbralización Automática")
    ...
    def custom_threshold(self):
        ...
        # Guardar el estado ANTES de aplicar el umbral
        self.save_processing_state(f"Umbral Personalizado {threshold_value}")
    ...
    def apply_threshold(self, threshold_value):
        ...
        # Siempre usar la imagen procesada actual como base
        if self.processed_image is not None:
            working_image = self.processed_image
        else:
        ...
        self.processed_image = binary_result
    ...
    def connected_components(self, connectivity):
        ...
        self.save_processing_state(f"Componentes Conexas Vecindad {connectivity}")
    ...
    def contour_objects(self):
        ...
        self.save_processing_state("Contornear Objetos")
        ...
        if self.processed_image is not None and self.processed_image is not self.binary_image:
            # Usar la imagen procesada actual como base
            if len(self.processed_image.shape) == 3:
                result_image = self.processed_image.copy()
            else:
                result_image = cv2.cvtColor(self.processed_image, cv2.COLOR_GRAY2BGR)
        else:
    ...
    def arithmetic_operation_scalar(self, operation):
        ...
        self.save_processing_state(f"Operación Aritmética {operation}")
    ...
    def arithmetic_operation_images(self, operation):
        ...
        self.save_processing_state(f"Operación entre Imágenes {operation}")
        ...
        try:
            if operation == 'add':
                ...
                self.set_message("Suma de imágenes aplicada.")
            elif operation == 'subtract':
                result = cv2.subtract(A_proc, B_proc);
                self.set_message("Resta de imágenes aplicada.")
            elif operation == 'multiply':
                result = cv2.multiply(A_proc, B_proc);
                self.set_message("Multiplicación de imágenes aplicada.")
            else:
    ...
    def logical_operation(self, operation):
        ...
        self.save_processing_state(f"Operación Lógica {operation}")
        ...
        if operation != 'not':
            ...
            if operation == 'and':
                ...
                self.set_message("Operación: AND aplicada.")
            elif operation == 'or':
                res = cv2.bitwise_or(A, B);
                self.set_message("Operación: OR aplicada.")
            elif operation == 'xor':
                res = cv2.bitwise_xor(A, B);
                self.set_message("Operación: XOR aplicada.")
            else:
                messagebox.showerror("Error", "Operación desconocida.")
                return
