...
from scipy import stats  # <--- DEBE SER AÑADIDO: Requerido para el filtro de moda

class ImageProcessingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Procesamiento de Imágenes") # Actualice el titulo
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(title_frame,
                                text="Procesamiento Digital de Imágenes", # Título actualizado
                                font=('Arial', 14, 'bold'))
        ...
        # --- carga / guardado / reset (Codigo de Cecilia) ---
        ...
        # --- Preprocesamiento (Codigo de Cecilia) ---
        preprocess_frame = ttk.LabelFrame(controls_frame, text="Preprocesamiento", padding=6)
        preprocess_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=6)
        ...
        # NUEVO AÑADIDO (Práctica de Ruido y Filtrado) ---
        
        noise_filter_frame = ttk.LabelFrame(controls_frame, text="Práctica: Ruido y Filtrado", padding=6)
        noise_filter_frame.grid(row=2, column=0, sticky=(tk.W, tk.E), pady=6)
        noise_filter_frame.columnconfigure((0,1), weight=1) # Distribuir columnas
        
        # --- Generación de Ruido
        noise_label = ttk.Label(noise_filter_frame, text="1. Generar Ruido:", font=('Arial', 9, 'bold'))
        noise_label.grid(row=0, column=0, columnspan=2, sticky=tk.W, pady=(0, 4))
        
        ttk.Button(noise_filter_frame, text="Añadir Ruido Sal y Pimienta", command=self.add_salt_pepper_noise).grid(row=1, column=0, padx=2, pady=3, sticky=(tk.W, tk.E))
        ttk.Button(noise_filter_frame, text="Añadir Ruido Gaussiano", command=self.add_gaussian_noise).grid(row=1, column=1, padx=2, pady=3, sticky=(tk.W, tk.E))

        # --- Filtros Lineales
        linear_label = ttk.Label(noise_filter_frame, text="2. Filtros Lineales (Pasa-bajas):", font=('Arial', 9, 'bold'))
        linear_label.grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=(8, 4))
        
        ttk.Button(noise_filter_frame, text="F. Promediador", command=self.apply_average_filter).grid(row=3, column=0, padx=2, pady=3, sticky=(tk.W, tk.E))
        ttk.Button(noise_filter_frame, text="F. Prom. Pesado", command=self.apply_weighted_average_filter).grid(row=3, column=1, padx=2, pady=3, sticky=(tk.W, tk.E))
        ttk.Button(noise_filter_frame, text="F. Gaussiano", command=self.apply_gaussian_filter).grid(row=4, column=0, columnspan=2, padx=2, pady=3, sticky=(tk.W, tk.E))

        # --- Filtros No Lineales
        nonlinear_label = ttk.Label(noise_filter_frame, text="3. Filtros No Lineales (de Orden):", font=('Arial', 9, 'bold'))
        nonlinear_label.grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=(8, 4))
        
        ttk.Button(noise_filter_frame, text="F. de Mediana", command=self.apply_median_filter).grid(row=6, column=0, padx=2, pady=3, sticky=(tk.W, tk.E))
        ttk.Button(noise_filter_frame, text="F. de Moda", command=self.apply_mode_filter).grid(row=6, column=1, padx=2, pady=3, sticky=(tk.W, tk.E))
        ttk.Button(noise_filter_frame, text="F. de Máximo", command=self.apply_max_filter).grid(row=7, column=0, padx=2, pady=3, sticky=(tk.W, tk.E))
        ttk.Button(noise_filter_frame, text="F. de Mínimo", command=self.apply_min_filter).grid(row=7, column=1, padx=2, pady=3, sticky=(tk.W, tk.E))
        
        # --- FIN: CÓDIGO AÑADIDO ---

        # --- Operaciones aritméticas con escalar (Codigo de Cecilia) ---
        arith_scalar_frame = ttk.LabelFrame(controls_frame, text="Operaciones Aritméticas con Escalar", padding=6)
        arith_scalar_frame.grid(row=3, column=0, sticky=(tk.W, tk.E), pady=6)
        ...
        ttk.Button(arith_scalar_frame, text="División", command=lambda: self.arithmetic_operation_scalar('divide')).grid(row=0, column=3, padx=2, pady=3)

        # --- Operaciones aritméticas entre imágenes (Codigo de Cecilia) ---
        arith_images_frame = ttk.LabelFrame(controls_frame, text="Operaciones Aritméticas entre Imágenes", padding=6)
        arith_images_frame.grid(row=4, column=0, sticky=(tk.W, tk.E), pady=6)
        ...
        ttk.Button(arith_images_frame, text="División", command=lambda: self.arithmetic_operation_images('divide')).grid(row=0, column=3, padx=2, pady=3)

        # --- Operaciones lógicas (Codigo de Cecilia) ---
        logic_frame = ttk.LabelFrame(controls_frame, text="Operaciones Lógicas", padding=6)
        logic_frame.grid(row=5, column=0, sticky=(tk.W, tk.E), pady=6)
        ...
        # --- Etiquetado de componentes (Codigo de Cecilia) ---
        labeling_frame = ttk.LabelFrame(controls_frame, text="Etiquetado de Componentes Conexas", padding=6)
        labeling_frame.grid(row=6, column=0, sticky=(tk.W, tk.E), pady=6)
        ...
        # ---- visualización controles (Codigo de Cecilia) -----------------
    ...
    # ----------------- carga / eliminación / display (Codigo de Cecilia) -----------------
    ...
    # ----------------- operaciones aritméticas (Codigo de Cecilia) -----------------
    def arithmetic_operation_scalar(self, operation):
        ...
        if self.binary_image is not None and self.processed_image is not None and np.array_equal(self.processed_image, self.binary_image):
             if operation == 'divide':
                 value = self.get_numeric_input("División por Escalar", "Ingresa el valor para dividir (no puede ser 0):", 2)
                 if value is None: return
                 self.set_message(f"División por escalar /{value} solicitada sobre imagen binarizada (sin cambios visibles).")
             self.display_image(self.processed_image, self.processed_canvas)
             self.show_histogram_auto()
             return
        ...
        if operation == 'add':
            ...
            if value == 0:
                ...
                return
            res = cv2.divide(src.astype(np.float32), float(value))
            self.processed_image = np.clip(res, 0, 255).astype(np.uint8)
            self.set_message(f"División por escalar /{value} aplicada.")
    ...
    def arithmetic_operation_images(self, operation):
        ...
        try:
            ...
            if operation == 'add':
                ...
                result = cv2.multiply(A_proc.astype(np.float32), B_proc.astype(np.float32)); 
                self.set_message("Multiplicación de imágenes aplicada.")
            
            elif operation == 'divide':
                A_float = A_proc.astype(np.float32)
                B_float = B_proc.astype(np.float32)
                
                B_float[B_float == 0] = 1e-5 
                
                result = cv2.divide(A_float, B_float, scale=255.0) 
                self.set_message("División de imágenes (A/B) * 255 aplicada.")
    ...
    def convert_to_grayscale(self):
        ...
        if src.ndim == 2:
            self.set_message("La imagen ya está en escala de grises.")
            return
    ...
    # ----------------- Componentes Conexas (Codigo de Cecilia) -----------------
    def connected_components(self, connectivity):
        if self.original_image is None:
            messagebox.showwarning("Advertencia", "Primero carga una imagen.")
            return
        if self.binary_image is None or not np.array_equal(self.processed_image, self.binary_image):
            self.set_message("Aplicando umbral automático (127) para componentes.")
            self.apply_threshold(127)
    ...
    def contour_objects(self):
        if self.original_image is None:
            ...
            return
        if self.binary_image is None or not np.array_equal(self.processed_image, self.binary_image):
            self.set_message("Aplicando umbral automático (127) para contornos.")
            self.apply_threshold(127)
            if self.binary_image is None:
                return
        ...
        # Aqui se usa la imagen procesada actual como base
        src_display = self.processed_image if self.processed_image is not None else self.original_image
        
        if len(src_display.shape) == 3:
            result_image = src_display.copy()
        else:
            result_image = cv2.cvtColor(src_display, cv2.COLOR_GRAY2BGR)
    ...
    # --- NUEVOS AÑADIDOS: MÉTODOS PARA PRÁCTICA DE FILTRADO ---

    def _get_grayscale_image(self):
        """
        Función auxiliar para asegurar que la imagen procesada esté en escala de grises.
        ...
        if src.ndim == 3:
            ...
            self.set_message("Imagen convertida a escala de grises para la operación.")
            return gray
        else:
            # Ya es grises
            return src.copy()
    ...
    def add_salt_pepper_noise(self):
        """Añade ruido 'sal y pimienta' a la imagen procesada."""
        img = self._get_grayscale_image()
        if img is None: return

        cantidad = self.get_numeric_input("Ruido Sal y Pimienta", 
                                          "Proporción de píxeles a afectar (ej. 0.05):", 
                                          0.05)
        if cantidad is None or cantidad < 0 or cantidad > 1:
            self.set_message("Cantidad inválida. Debe ser entre 0 y 1.")
            return
            
        salida = img.copy()
        num_pixeles = int(cantidad * img.size)
        
        # Añadir Ruido Sal (blanco) [cite: 111-116]
        coords_sal = [np.random.randint(0, i - 1, num_pixeles // 2) for i in img.shape]
        salida[coords_sal[0], coords_sal[1]] = 255
        
        # Añadir Ruido Pimienta (negro) [cite: 117-118]
        coords_pimienta = [np.random.randint(0, i - 1, num_pixeles // 2) for i in img.shape]
        salida[coords_pimienta[0], coords_pimienta[1]] = 0
        
        self.processed_image = salida
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Ruido Sal y Pimienta (cantidad={cantidad}) añadido.")

    def add_gaussian_noise(self):
        """Añade ruido gaussiano a la imagen procesada."""
        img = self._get_grayscale_image()
        if img is None: return

        media = self.get_numeric_input("Ruido Gaussiano", "Valor de la media:", 0)
        sigma = self.get_numeric_input("Ruido Gaussiano", "Valor de Sigma (desviación):", 25)
        
        if media is None or sigma is None:
            self.set_message("Operación cancelada.")
            return
            
        # Generar ruido gaussiano [cite: 140]
        gauss = np.random.normal(media, sigma, img.shape).astype('int16')
        
        # Añadir ruido y asegurar que esté en el rango [0, 255]
        salida_int = img.astype('int16') + gauss
        salida = np.clip(salida_int, 0, 255).astype('uint8')
        
        self.processed_image = salida
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Ruido Gaussiano (media={media}, sigma={sigma}) añadido.")

    # --- 2. Filtros Lineales ---

    def apply_average_filter(self):
        """Aplica un filtro promediador (blur)[cite: 38]."""
        img = self._get_grayscale_image()
        if img is None: return
        
        k_size = simpledialog.askinteger("Filtro Promediador", "Tamaño del kernel (impar, ej. 5):", initialvalue=5)
        if k_size is None or k_size % 2 == 0 or k_size < 1:
            self.set_message("Tamaño de kernel inválido. Debe ser un número impar positivo.")
            return
        
        self.processed_image = cv2.blur(img, (k_size, k_size)) # [cite: 159]
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro promediador aplicado (kernel={k_size}x{k_size}).")
    
    def apply_weighted_average_filter(self):
        """Aplica un filtro promediador pesado (con centro de 5)[cite: 39]."""
        img = self._get_grayscale_image()
        if img is None: return
        
        # Kernel 3x3 pesado como en la Figura 5 [cite: 164]
        kernel = np.array([[1, 1, 1], 
                           [1, 5, 1], 
                           [1, 1, 1]], dtype=np.float32) / 13.0
                           
        self.processed_image = cv2.filter2D(img, -1, kernel) # [cite: 165]
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message("Filtro promediador pesado (3x3, centro=5) aplicado.")

    def apply_gaussian_filter(self):
        """Aplica un filtro Gaussiano[cite: 40]."""
        img = self._get_grayscale_image()
        if img is None: return

        k_size = simpledialog.askinteger("Filtro Gaussiano", "Tamaño del kernel (impar, ej. 5):", initialvalue=5)
        if k_size is None or k_size % 2 == 0 or k_size < 1:
            self.set_message("Tamaño de kernel inválido. Debe ser un número impar positivo.")
            return
            
        sigma = self.get_numeric_input("Filtro Gaussiano", "Valor de Sigma X (ej. 1):", 1)
        if sigma is None: return

        self.processed_image = cv2.GaussianBlur(img, (k_size, k_size), sigma) # [cite: 191]
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro Gaussiano aplicado (kernel={k_size}x{k_size}, sigma={sigma}).")

    # --- 3. Filtros No Lineales ---

    def apply_median_filter(self):
        """Aplica un filtro de mediana[cite: 43]."""
        img = self._get_grayscale_image()
        if img is None: return

        k_size = simpledialog.askinteger("Filtro de Mediana", "Tamaño del kernel (impar, ej. 5):", initialvalue=5)
        if k_size is None or k_size % 2 == 0 or k_size < 1:
            self.set_message("Tamaño de kernel inválido. Debe ser un número impar positivo.")
            return

        self.processed_image = cv2.medianBlur(img, k_size) # [cite: 201]
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro de Mediana aplicado (kernel={k_size}).")

    def _filtro_orden_manual(self, img, kernel_size, operation):
        """
        Función auxiliar manual para filtros de orden (moda, max, min).
        Basado en la lógica del filtro de moda [cite: 209-221].
        """
        salida = np.copy(img)
        h, w = img.shape
        pad = kernel_size // 2
        # Usar padding de ceros como en la práctica [cite: 214]
        imagen_padded = np.pad(img, pad, mode='constant', constant_values=0)
        
        for i in range(h):
            for j in range(w):
                window = imagen_padded[i:i + kernel_size, j:j + kernel_size]
                
                if operation == 'moda':
                    # Aplanar la ventana para stats.mode [cite: 218]
                    flat_window = window.flatten()
                    moda = stats.mode(flat_window)[0] # [cite: 219]
                    salida[i, j] = moda
                elif operation == 'max':
                    salida[i, j] = np.max(window)
                elif operation == 'min':
                    salida[i, j] = np.min(window)
        return salida

    def apply_mode_filter(self):
        """Aplica un filtro de moda (implementación manual)[cite: 44]."""
        img = self._get_grayscale_image()
        if img is None: return

        k_size = simpledialog.askinteger("Filtro de Moda", "Tamaño del kernel (impar, ej. 3):", initialvalue=3)
        if k_size is None or k_size % 2 == 0 or k_size < 1:
            self.set_message("Tamaño de kernel inválido. Debe ser un número impar positivo.")
            return

        self.processed_image = self._filtro_orden_manual(img, k_size, 'moda')
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro de Moda aplicado (kernel={k_size}).")

    def apply_max_filter(self):
        """Aplica un filtro de máximo (implementación manual)[cite: 45]."""
        img = self._get_grayscale_image()
        if img is None: return

        k_size = simpledialog.askinteger("Filtro de Máximo", "Tamaño del kernel (impar, ej. 3):", initialvalue=3)
        if k_size is None or k_size % 2 == 0 or k_size < 1:
            self.set_message("Tamaño de kernel inválido. Debe ser un número impar positivo.")
            return

        self.processed_image = self._filtro_orden_manual(img, k_size, 'max')
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro de Máximo aplicado (kernel={k_size}).")

    def apply_min_filter(self):
        """Aplica un filtro de mínimo (implementación manual)[cite: 45]."""
        img = self._get_grayscale_image()
        if img is None: return

        k_size = simpledialog.askinteger("Filtro de Mínimo", "Tamaño del kernel (impar, ej. 3):", initialvalue=3)
        if k_size is None or k_size % 2 == 0 or k_size < 1:
            self.set_message("Tamaño de kernel inválido. Debe ser un número impar positivo.")
            return

        self.processed_image = self._filtro_orden_manual(img, k_size, 'min')
        self.display_image(self.processed_image, self.processed_canvas)
        self.show_histogram_auto()
        self.set_message(f"Filtro de Mínimo aplicado (kernel={k_size}).")


    # --- FIN: MÉTODOS AÑADIDOS ---


    # ----------------- histogramas (Codigo de Cecilia) -----------------
